 ...
 */
@Service
public class TaskStatisticsBatch {
	...
	private static Logger logger = LogManager.getLogger(TaskStatisticsBatch.class);
	
	@Scheduled(initialDelay=2*60*1000,fixedDelay = 5*60*1000)   //服务启动后2分钟执行，然后执行后5分钟在执行
	public void timerTask(){
		//批次
		doStatisticsWork(StatisticsType.STATISTICS_BATCH.getMessage(), StatisticsType.STATISTICS_BATCH.getValue());
		//批次封包
		doStatisticsWork(StatisticsType.STATISTICS_BATCH_BOX.getMessage(), StatisticsType.STATISTICS_BATCH_BOX.getValue());
	}
	
	/**
	 * 执行具体的统计工作:从sorting_temp（分拣数据）、box_temp（封包数据）表中查询数据插入到：report_sorting表中；
	 * @param tableName 
	 * @param type  9，批次统计；10，批次封包；
	 */
